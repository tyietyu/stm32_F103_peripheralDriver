...
主控实际为 **STM32F407V(E-G)Tx**（仓库名中的 F103 属历史遗留）。

### 初始化顺序

```c
delay_init();                 /* 软件 I2C 位延时依赖，必须最先执行 */
AS5600_Init(&sensor);         /* 返回 0 表示磁体正常且 CONF 写入已回读校验 */
