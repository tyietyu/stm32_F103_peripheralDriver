...
/**
 * @description: FOC 电流闭环控制实现
 * @param {FOC_T} *hfoc foc句柄
 * @param {float} Iq_target 目标 Q 轴电流 (A)
 * @param {float} angle_el 当前电角度 (rad)
 */
void FOC_SetCurrent(FOC_T *hfoc, float Iq_target, float angle_el)
{
  hfoc->target_Iq = Iq_target;
  hfoc->target_Id = 0.0f; // 恒定为 0 以获得最大扭矩
  ...
  // 1. Clarke 变换 (三相到二相静止)
  hfoc->I_alpha = hfoc->I_u;
  hfoc->I_beta = (hfoc->I_u + 2.0f * hfoc->I_v) / sqrt(3.0f);

  // 2. Park 变换 (二相静止到二相旋转)
  hfoc->I_d = hfoc->I_alpha * cos_el + hfoc->I_beta * sin_el;
  hfoc->I_q = -hfoc->I_alpha * sin_el + hfoc->I_beta * cos_el;

  // 3. 电流环 PID 计算 (输出为 Vd 和 Vq)
  float Vd = PID_Calc(&hfoc->pid_id, hfoc->target_Id - hfoc->I_d);
  float Vq = PID_Calc(&hfoc->pid_iq, hfoc->target_Iq - hfoc->I_q);

  // 4. 反 Park 变换 (二相旋转到二相静止电压)
  float Ualpha = Vd * cos_el - Vq * sin_el;
  float Ubeta  = Vd * sin_el + Vq * cos_el;

  // 5. 逆 Clark 变换生成三相占空比 (写入 PWM)
  float Ua = Ualpha + hfoc->voltage_power_supply / 2.0f;
  float Ub = (sqrt(3.0f) * Ubeta - Ualpha) / 2.0f + hfoc->voltage_power_supply / 2.0f;
  float Uc = (-Ualpha - sqrt(3.0f) * Ubeta) / 2.0f + hfoc->voltage_power_supply / 2.0f;

  _setPwm(hfoc, Ua, Ub, Uc);
}

// 修改初始化函数以包含电流 PID 初始化
void FOC_CurrentLoop_Init(FOC_T *hfoc)
{
  PID_Init(&hfoc->pid_id, 0.5f, 10.0f, 0.0f, 10000.0f, hfoc->voltage_power_supply / 2.0f);
  PID_Init(&hfoc->pid_iq, 0.5f, 10.0f, 0.0f, 10000.0f, hfoc->voltage_power_supply / 2.0f);
}
