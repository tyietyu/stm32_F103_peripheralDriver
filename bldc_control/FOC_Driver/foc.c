 ...
 */
#define FOC_OBSERVER_OMEGA_N     (2.0f * FOC_PI * FOC_OBSERVER_BANDWIDTH_HZ)
                                     ...
                                     float *ud_applied, float *uq_applied)
{
  ...
  hfoc->voltage_clip_uq = requested_uq - uq;
                                        ...
                                        uint32_t timeout_ms)
{
  ...
  FOC_AlignmentSetVoltage(hfoc, voltage, angle_el);
                                 ...
                                 float movement_threshold)
{
  ...
  if (result != FOC_RESULT_OK)
  {
    FOC_AlignmentSetVoltage(hfoc, 0.0f, FOC_THREE_PI_2);
    return result;
  }
                                   ...
                                   void *pid_id, void *pid_iq, uint32_t now)
{
  ...
  if ((pid_id == NULL) || (pid_iq == NULL))
  {
    return FOC_ERROR_PID_NOT_BOUND;
  }
  if (FOC_IsCurrentSampleValid(hfoc) == 0U)
  {
    return FOC_ERROR_PWM_SAMPLE_INVALID;
  }
