...
void CAW_LOG_Write(const char* fmt, CAW_LOG_LEVEL level, const char* file, int line, const char* func, ...) 
{
  ...
  if (level == LEVEL_DEBUG) 
  {
    ...
    {
      snprintf(buf, sizeof(buf),"\033[1;36mCAW-[DEBUG] <%s> | <%d> | <%s>: %s\033[m\r\n", file,line, func, tmp);
    }
    else
    {
  ...
  else if (level == LEVEL_INFO)
  {
    if (G_CAW_LOG_Instance.enable_color)
    {
      snprintf(buf, sizeof(buf), "\033[1;32mCAW-[INFO] <%s> | <%d> | <%s>: %s\033[m\r\n", file, line, func, tmp);
    }
    else
    {
  ...
  else if (level == LEVEL_WARN)
  {
    if (G_CAW_LOG_Instance.enable_color)
    {
      snprintf(buf, sizeof(buf), "\033[1;33mCAW-[WARN] <%s> | <%d> | <%s>: %s\033[m\r\n", file, line, func, tmp);
    }
    else
    {
  ...
  else if (level == LEVEL_ERROR)
  {
    if (G_CAW_LOG_Instance.enable_color)
    {
      snprintf(buf, sizeof(buf), "\033[1;31mCAW-[ERROR] <%s> | <%d> | <%s>: %s\033[m\r\n", file, line, func, tmp);
    }
    else
    {
