...
{
  if (CDC_RxFlag == 0) return 0;
  
  *len = CDC_RxLen;
  for (uint32_t i = 0; i < CDC_RxLen; i++)
  {
    buf[i] = UserRxBufferHS[i];
  }
  CDC_RxFlag = 0;
  CDC_RxLen = 0;
  return 1;
...
/**
 * @brief 检查USB CDC发送是否完成
 * @return 1: 完成, 0: 发送中
 */
uint8_t CDC_IsTxComplete(void)
{
  return CDC_TxComplete;
}

/**
 * @brief 阻塞等待USB CDC发送完成
 * @param timeout 超时时间(ms)
 * @return 0: 成功, 1: 超时
 */
uint8_t CDC_WaitTxComplete(uint32_t timeout)
{
  while (CDC_TxComplete == 0 && timeout > 0)
  {
    HAL_Delay(1);
    timeout--;
  }
